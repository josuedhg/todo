...
static void test_done_command_no_param(void **state)
{
	...
	assert_int_not_equal(buffer_size, 0);
	assert_non_null(strstr(buffer, "Usage: done [task id] \n"));
	free(buffer);
}
...
static void test_done_command_cannot_save_task(void **state)
{
	(void)state; /* unused */
	char *params[] = {"done", "1"};
	char *buffer = NULL;
...
int main(int argc, char *argv[])
{
	struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(test_done_command_cannot_save_task),
