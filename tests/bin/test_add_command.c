...
static void test_add_command_no_param(void **state)
{
	...
	instrument_stderr();
	assert_int_equal(add_command.command_handle(0, NULL), -1);
	buffer_size = get_stderr_buffer(&buffer);
	deinstrument_stderr();
...
static void test_add_command_cannot_access_file(void **state)
{
	(void)state; /* unused */
	...
	assert_int_equal(add_command.command_handle(2, params), -1);
	buffer_size = get_stderr_buffer(&buffer);
	deinstrument_stderr();
...
static void test_add_command_cannot_read_file(void **state)
{
	(void)state; /* unused */
	char *params[] = {"add", "task"};
	char *buffer = NULL;
...
static void test_add_command_success(void **state)
{
	...
	assert_int_equal(add_command.command_handle(2, params), 0);
	buffer_size = get_stderr_buffer(&buffer);
	deinstrument_stderr();
...
static void test_add_command_success_multi_word(void **state)
{
	...
	expect_value(__wrap_create_new_task, priority, TASK_PRIORITY_LOW);
	will_return(__wrap_todo_save_tasks, 0);
	assert_int_equal(add_command.command_handle(4, params), 0);
	buffer_size = get_stderr_buffer(&buffer);
	deinstrument_stderr();
...
int main(int argc, char *argv[])
{
	struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(test_add_command_cannot_access_file),
		cmocka_unit_test(test_add_command_cannot_read_file),
